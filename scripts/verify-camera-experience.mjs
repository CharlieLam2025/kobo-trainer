...
import { readAllSource, readSourceFile } from './read-sources.mjs';
...
for (const marker of [
  ...
  assert.ok(source.includes(marker), `src missing camera experience marker: ${marker}`);
}

const cameraHook = readSourceFile('src/hooks/use-camera.jsx');
...
assert.ok(cameraHook.includes('CAMERA_PIPELINE_FPS'), 'camera pipeline should declare target FPS');
assert.ok(cameraHook.includes('getPipeline2dContext') || cameraHook.includes('desynchronized'), 'pipeline should use optimized 2d context');
assert.ok(cameraHook.includes('document.hidden') || cameraHook.includes('visibilitychange'), 'pipeline should pause when tab is hidden');
