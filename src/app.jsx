...
// DeepSeek 模拟发布：假装发到平台，AI 给评分 + 真实观众评论 + 互动数据
async function deepseekVirtualPublish({ apiKey, topic, transcript, durationSec, platform = null }) {
  const sys = '你是一位社交媒体老手，熟悉抖音 / 小红书 / 视频号 / 知乎的真实评论生态。你要模拟一条短视频在被推荐后的真实反应 —— 包括点赞、收藏、评论、转发，以及不同立场的观众弹评。同时基于内容质量做客观评分。';
  const platformLine = platform
    ? `\n用户指定要发到：${platform}（按这个平台的调性来模拟）`
    : '\n请你判断最适合的平台（抖音 / 小红书 / 视频号 / 知乎 四选一）';
  const usr = `这是一条短视频口播：
话题：《${topic || '自由口播'}》
时长：${Math.round(durationSec)} 秒
转录稿："""
${String(transcript || '').slice(0, 1800) || '（用户没有转录稿，请基于话题做合理推断）'}
"""${platformLine}

假装这条已经发出去了 · 给一份"像真社交软件那样"的模拟反馈，严格按 JSON 返回（不要 markdown 代码块、不要解释）：
{
  "platform": "抖音 | 小红书 | 视频号 | 知乎 之一",
  "platform_reason": "≤25 字理由",
  "author": { "avatar": "1 个 emoji（动物/水果/食物等可爱）", "name": "你的虚拟账号名（≤8 字 · 跟内容调性匹配）" },
  "headline": "≤22 字标题（观众视角 · 用钩子或疑问体）",
  "estimated": {
    "likes": 数字（基于内容质量 + 话题热度 · 50-200000）,
    "comments": 数字,
    "shares": 数字,
    "saves": 数字（收藏数 · 干货类高 / 娱乐类低）,
    "watch_through": 0-100 整数（完播率%）
  },
  "score": 0-100 整数（综合评分 · 60 及格 / 80 优秀 / 90+ 爆款潜质）,
  "score_breakdown": {
    "hook": 0-10 整数（钩子强度）,
    "content": 0-10 整数（内容质量）,
    "resonance": 0-10 整数（情绪共鸣）,
    "virality": 0-10 整数（传播性）,
    "completion": 0-10 整数（完播预期）
  },
  "vibe": "viral | niche | divisive | boring | wholesome 之一",
  "trending_curve": [3-5 个 0-100 数字 · 表示发布后 24h 热度趋势],
  "comments": [
    {
      "stance": "理解派 | 吃瓜 | 挑刺 | 同行 | 围观 | 玩梗 之一",
      "avatar": "1 个 emoji（评论者头像）",
      "user": "网名（短/有特色/可含 emoji · ≤10 字）",
      "text": "≤40 字 · 自然 · 可有错别字或网络梗",
      "time": "时间标签（如 '刚刚' '3 分钟前' '1 小时前' '半天前'）",
      "likes": 数字（这条评论的赞数 · 0-500）
    }
  ],
  "potential_pitfall": "≤25 字 · 可能踩雷的点（无可空）",
  "if_pushed_to_top": "≤25 字 · 被推流后最可能炸的部分"
}

要求：
- comments 6-10 条 · 立场多样 · 至少 1 条质疑 / 1 条玩梗 / 1 条真心捧
- 评论要"网友味"，不要客套 ("写得真好"这种话不要)
- 评论 time 字段从"刚刚"到"半天前"散开，制造时间感
- 评论 likes 字段呈金字塔分布（高赞 1-2 条到 100+，其他 0-30）
- estimated 不要一律往高估，差内容就给真实低数据（甚至 likes < 100）
- score 要诚实：转录稿过短/无意义 → score < 30 + vibe='boring' + 数据 < 500 + 评论多写"看不懂在讲啥"
- author.name 要跟内容调性匹配（写认知的话像"思考の鱼"，写生活的话像"猫姐和橘子"）
- trending_curve 表示热度曲线（如 [10, 45, 80, 60, 35] 表示先涨后跌）
- 如果挑刺，用具体的"哪里没说清楚"，不是空骂`;
...
const DoneView = ({ blob, contextLabel, duration = 0, onRetry, onNew, extra, transcript = '' }) => {
  ...
  return (
    <div className="fade-in">
      ...
      {/* 🌐 模拟发布：假装发出去 · 评分 + 评论 + 数据 */}
      {blob && <VirtualPublish topic={contextLabel} durationSec={duration} initialTranscript={transcript} filename={saveStatus.filename} />}
...
// ============ 模拟发布：假装发到平台 · AI 评分 + 评论 + 数据 ============
const VIBE_META = {
  viral:     { emoji:'🔥', label:'要爆',     color:'#A30236', bg:'#FBEFF2' },
  niche:     { emoji:'💎', label:'小众精',   color:'#7c3aed', bg:'#F3E8FF' },
  divisive:  { emoji:'⚔️', label:'撕裂',     color:'#F1A23F', bg:'#FEF3C7' },
  boring:    { emoji:'💤', label:'平',       color:'#737373', bg:'#F5F5F4' },
  wholesome: { emoji:'💚', label:'温暖',     color:'#10b981', bg:'#D1FAE5' },
};

const STANCE_COLORS = {
  '理解派': { color:'#10b981', bg:'#D1FAE5' },
  '吃瓜':   { color:'#A0775B', bg:'#F5F0E8' },
  '挑刺':   { color:'#A30236', bg:'#FBEFF2' },
  '同行':   { color:'#7c3aed', bg:'#F3E8FF' },
  '围观':   { color:'#737373', bg:'#F5F5F4' },
  '玩梗':   { color:'#F1A23F', bg:'#FEF3C7' },
};

// 平台视觉主题
const PLATFORM_THEMES = {
  '抖音':   { headerBg:'#000000', headerText:'#fff', accent:'#FE2C55', accentBg:'#1a1a1a', tag:'抖音', emoji:'🎵' },
  '小红书': { headerBg:'#FF2442', headerText:'#fff', accent:'#FF2442', accentBg:'#FFF1F2', tag:'小红书', emoji:'📕' },
  '视频号': { headerBg:'#07C160', headerText:'#fff', accent:'#07C160', accentBg:'#ECFDF5', tag:'视频号', emoji:'💚' },
  '知乎':   { headerBg:'#0084FF', headerText:'#fff', accent:'#0084FF', accentBg:'#EFF6FF', tag:'知乎', emoji:'💭' },
};
...
// 数字动画组件：从 0 飘到目标值
const AnimatedNumber = ({ target = 0, duration = 1200, format = (n) => String(Math.round(n)) }) => {
...
const ScoreBadge = ({ score = 0 }) => {
  ...
  const color = score >= 90 ? '#A30236' : score >= 80 ? '#F1A23F' : score >= 70 ? '#10b981' : score >= 60 ? '#7c3aed' : '#737373';
  return (
    <div className="flex flex-col items-center justify-center px-4 py-3" style={{
      borderRadius:'4px',
      background: `linear-gradient(135deg, ${color}15, ${color}05)`,
      border: `2px solid ${color}`,
    }}>
      <div className="text-[8px] tracking-[0.2em] uppercase font-bold opacity-60" style={{color}}>SCORE</div>
      <div className="font-display font-bold text-4xl tabular-nums leading-none mt-0.5" style={{color}}>
        <AnimatedNumber target={score} />
      </div>
      <div className="text-[10px] mt-1 font-bold tracking-wider" style={{color}}>{grade} 级 · /100</div>
    </div>
  );
};
...
// 渲染模拟发布结果为可分享海报（1080×1920）· 返回 Blob
async function renderVirtualPublishPoster(result, contextLabel = '') {
  const W = 1080, H = 1920;
  const canvas = document.createElement('canvas');
  canvas.width = W; canvas.height = H;
  const ctx = canvas.getContext('2d');
  const theme = PLATFORM_THEMES[result.platform] || PLATFORM_THEMES['抖音'];
  const author = result.author || { avatar:'🦊', name:'你的账号' };
  ...
  ctx.textAlign = 'left';
  ctx.font = `bold 50px ${POSTER_FONT}`;
  ctx.fillText(`${theme.emoji} 模拟发布 · ${result.platform}`, 60, 105);
  ctx.font = `28px ${POSTER_FONT}`;
  ctx.fillStyle = theme.headerText + 'cc'; // semi-transparent
  ctx.fillText(result.platform_reason || '', 60, 150);

  // ===== Author + Headline =====
  let y = 240;
  ctx.font = `100px ${POSTER_FONT}`;
  ctx.textAlign = 'left';
  ctx.fillStyle = '#000';
  ctx.fillText(author.avatar || '🦊', 60, y + 80);
  ctx.font = `bold 44px ${POSTER_FONT}`;
  ctx.fillStyle = '#1c1917';
  ctx.fillText(author.name || '你的账号', 200, y + 40);
  ctx.font = `26px ${POSTER_FONT}`;
  ctx.fillStyle = '#78716c';
  ctx.fillText(`刚刚 · 模拟发布到 ${result.platform}`, 200, y + 80);
  y += 130;

  // Headline (wrap)
  ctx.fillStyle = '#1c1917';
  ctx.font = `bold 52px ${POSTER_FONT}`;
  const hlLines = wrapText(ctx, `"${result.headline || ''}"`, W - 120);
  for (const line of hlLines.slice(0, 3)) {
  ...
  // ===== Score block =====
  const scoreColor = result.score >= 90 ? '#A30236' : result.score >= 80 ? '#F1A23F' : result.score >= 70 ? '#10b981' : result.score >= 60 ? '#7c3aed' : '#737373';
  // Score box
  drawRoundedRect(ctx, 60, y, 280, 240, 12, scoreColor + '15');
  ctx.strokeStyle = scoreColor;
  ctx.lineWidth = 4;
  ctx.stroke();
  ctx.fillStyle = scoreColor;
  ctx.textAlign = 'center';
  ctx.font = `bold 24px ${POSTER_FONT}`;
  ctx.fillText('SCORE', 200, y + 50);
  ctx.font = `bold 130px ${POSTER_FONT}`;
  ctx.fillText(String(result.score ?? 0), 200, y + 175);
  ctx.font = `bold 22px ${POSTER_FONT}`;
  const grade = result.score >= 90 ? 'S' : result.score >= 80 ? 'A' : result.score >= 70 ? 'B' : result.score >= 60 ? 'C' : 'D';
  ctx.fillText(`${grade} 级 · /100`, 200, y + 215);

  // Vibe badge (next to score)
  if (result.vibe && VIBE_META[result.vibe]) {
    const v = VIBE_META[result.vibe];
    drawRoundedRect(ctx, 380, y, 290, 70, 8, v.bg);
    ctx.strokeStyle = v.color;
    ctx.lineWidth = 3;
    ctx.stroke();
    ctx.fillStyle = v.color;
    ctx.textAlign = 'left';
    ctx.font = `bold 32px ${POSTER_FONT}`;
    ctx.fillText(`${v.emoji} ${v.label}`, 405, y + 47);
  }

  // 5-dim breakdown bars
  ...
  for (const [name, val] of dims) {
    ctx.font = `26px ${POSTER_FONT}`;
    ctx.fillStyle = '#78716c';
    ctx.fillText(name, 380, by + 8);
    // bar bg
    ctx.fillStyle = '#e7e5e4';
    drawRoundedRect(ctx, 460, by - 12, 130, 12, 6, '#e7e5e4');
    ctx.fillStyle = theme.accent;
    drawRoundedRect(ctx, 460, by - 12, 130 * (val / 10), 12, 6, theme.accent);
    ctx.fillStyle = '#1c1917';
    ctx.font = `bold 24px ${POSTER_FONT}`;
    ctx.textAlign = 'right';
    ctx.fillText(String(val), 670, by + 8);
    ctx.textAlign = 'left';
    by += 28;
  }
  y += 280;
  ...
  const metrics = [
    ['❤️', formatCount(est.likes || 0), '点赞'],
    ['💬', formatCount(est.comments || 0), '评论'],
    ['⭐', formatCount(est.saves || 0), '收藏'],
    ['📤', formatCount(est.shares || 0), '转发'],
    ['▶️', `${est.watch_through || 0}%`, '完播'],
  ];
  const mw = (W - 120) / 5;
  ctx.fillStyle = '#fafaf9';
  drawRoundedRect(ctx, 60, y, W - 120, 160, 12, '#fafaf9');
  ctx.strokeStyle = '#e7e5e4';
  ctx.lineWidth = 2;
  ctx.stroke();
  metrics.forEach(([icon, val, lbl], i) => {
    const cx = 60 + mw * i + mw / 2;
    ctx.textAlign = 'center';
    ctx.font = `42px ${POSTER_FONT}`;
    ctx.fillStyle = '#1c1917';
    ctx.fillText(icon, cx, y + 50);
    ctx.font = `bold 36px ${POSTER_FONT}`;
    ctx.fillStyle = i === 0 ? theme.accent : (i === 2 ? '#F1A23F' : '#1c1917');
    ctx.fillText(val, cx, y + 105);
    ctx.font = `22px ${POSTER_FONT}`;
    ctx.fillStyle = '#78716c';
    ctx.fillText(lbl, cx, y + 140);
  });
  y += 200;
  ...
  if (result.comments?.length) {
    ...
    ctx.font = `bold 32px ${POSTER_FONT}`;
    ctx.fillStyle = '#1c1917';
    ctx.fillText(`💬 模拟评论 · 共 ${formatCount(est.comments || result.comments.length)} 条`, 60, y);
    y += 50;
    // Show top 3 comments (by likes)
    const topComments = [...result.comments]
      .sort((a, b) => (b.likes || 0) - (a.likes || 0))
      .slice(0, 3);
    for (const c of topComments) {
      const meta = STANCE_COLORS[c.stance] || STANCE_COLORS['围观'];
      drawRoundedRect(ctx, 60, y, W - 120, 180, 10, '#fff');
      ctx.strokeStyle = '#e7e5e4';
      ctx.lineWidth = 2;
      ctx.stroke();
      // Avatar
      ...
      ctx.textAlign = 'left';
      ctx.fillStyle = '#000';
      ctx.fillText(c.avatar || '👤', 90, y + 70);
      // User + stance + time
      ctx.font = `bold 28px ${POSTER_FONT}`;
      ctx.fillStyle = '#1c1917';
      ctx.fillText(c.user || '路人', 180, y + 50);
      // Stance pill
      const stanceText = c.stance || '围观';
      ctx.font = `bold 20px ${POSTER_FONT}`;
      const stW = ctx.measureText(stanceText).width + 16;
      drawRoundedRect(ctx, 180 + ctx.measureText(c.user || '路人').width + 100, y + 28, stW, 30, 4, meta.bg);
      ctx.fillStyle = meta.color;
      ctx.fillText(stanceText, 180 + ctx.measureText(c.user || '路人').width + 108, y + 50);
      // Time
      ctx.font = `22px ${POSTER_FONT}`;
      ctx.fillStyle = '#78716c';
      ctx.fillText(c.time || '刚刚', 180 + ctx.measureText(c.user || '路人').width + 100 + stW + 20, y + 50);
      // Text (wrap to 2 lines)
      ctx.font = `28px ${POSTER_FONT}`;
      ctx.fillStyle = '#292524';
      const tLines = wrapText(ctx, c.text || '', W - 240);
      let ty = y + 100;
      for (const tl of tLines.slice(0, 2)) {
        ctx.fillText(tl, 180, ty);
        ty += 38;
      }
      // Likes
      if ((c.likes || 0) > 0) {
        ctx.font = `22px ${POSTER_FONT}`;
        ctx.fillStyle = '#78716c';
        ctx.fillText(`❤️ ${formatCount(c.likes)} · 回复`, 180, y + 165);
      }
      y += 200;
    }
  }
  ...
  ctx.font = `bold 28px ${POSTER_FONT}`;
  ctx.fillStyle = '#fff';
  ctx.fillText('🎙️ 口播练习器 · 模拟发布（AI 生成 · 非真实数据）', W / 2, footerY + 42);
  ctx.font = `22px ${POSTER_FONT}`;
...
const VirtualPublish = ({ topic, durationSec = 0, initialTranscript = '', filename = null }) => {
  const settings = useSettings();
  const [result, setResult]   = useState(null);
  const [loading, setLoading] = useState(false);
  const [error, setError]     = useState('');
  const [expanded, setExpanded] = useState(false);
  const [chosenPlatform, setChosenPlatform] = useState(null); // 用户主动选择的平台 (可空 → AI 自动)
  const [publishStage, setPublishStage] = useState('idle'); // idle | publishing | done
  // 海报生成
  ...
  const makePoster = async () => {
    ...
    try {
      const blob = await renderVirtualPublishPoster(result, topic);
      setPosterBlob(blob);
    } catch (e) {
      console.error('poster error', e);
      setError('生成海报失败：' + (e?.message || e));
    } finally {
      setPosterLoading(false);
    }
  };

  // 如果对应的 savedFile 已经有数据，直接读取
  useEffect(() => {
    if (!filename) return;
    const file = (settings.savedFiles || []).find(f => f.filename === filename);
    if (file?.virtualPub) {
      setResult(file.virtualPub);
      setExpanded(true);
      setPublishStage('done');
    }
  }, [filename, settings.savedFiles]);

  const run = async () => {
    setLoading(true); setError(''); setPublishStage('publishing');
    try {
      const r = await deepseekVirtualPublish({
        apiKey: settings.apiKey,
        topic: topic || '',
      ...
      // 给"发布动画"一个最小展示时间（即使 AI 极快返回也至少 2 秒）
      const minDelay = new Promise(res => setTimeout(res, 2000));
      await minDelay;
      setResult(r);
      setPublishStage('done');
      if (filename && settings.updateSavedFile) {
        settings.updateSavedFile(filename, { virtualPub: r });
      }
    } catch (e) {
      setError(e.message || String(e));
  ...
  const reset = () => {
    ...
    if (filename && settings.updateSavedFile) {
      settings.updateSavedFile(filename, { virtualPub: null });
    }
  };
  ...
  if (!expanded && !result) {
    return (
      <Card className="p-4 mb-4 border-l-[3px] border-purple-400 cursor-pointer hover:bg-purple-50/50 transition-colors"
        onClick={() => setExpanded(true)}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 bg-purple-100 text-purple-700 flex items-center justify-center" style={{borderRadius:"3px"}}>
              <Icon name="sparkle" size={16} strokeWidth={1.7} />
            </div>
            <div>
              <div className="font-display font-bold text-stone-900 text-[15px]">🌐 模拟发布</div>
              <div className="text-[11px] text-stone-500 mt-0.5">假装发到抖音 / 小红书 · AI 给评分 + 真实观众评论 + 数据</div>
            </div>
          </div>
          <div className="text-purple-700 text-xs font-bold">展开 →</div>
        </div>
      </Card>
    );
  }
  ...
  // 发布中动画
  if (publishStage === 'publishing') {
    const platformName = chosenPlatform || '推流系统';
    return (
      <Card className="p-8 mb-4 border-l-[3px] border-purple-400 bg-gradient-to-br from-purple-50 to-stone-50">
        <div className="flex flex-col items-center justify-center min-h-[200px]">
          <div className="relative mb-6">
            <div className="text-6xl animate-pulse">🚀</div>
            <div className="absolute -inset-2 border-2 border-purple-400/30 rounded-full animate-ping" />
          </div>
          <div className="font-display font-bold text-stone-900 text-lg mb-1">正在发布到 {platformName}...</div>
          <div className="text-[11px] text-stone-500 mt-2 tracking-wider">观众正在赶来 · 评论区在生成 · 算法在打分</div>
  ...
  if (!result) {
    return (
      <Card className="p-6 mb-4 border-l-[3px] border-purple-400">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-2.5">
            <div className="w-8 h-8 bg-purple-100 text-purple-700 flex items-center justify-center" style={{borderRadius:"3px"}}>
              <Icon name="sparkle" size={16} strokeWidth={1.7} />
            ...
            <div>
              <h3 className="font-display font-bold text-stone-900 text-[16px] m-0">🌐 模拟发布</h3>
              <div className="text-[9px] tracking-wider text-stone-400 mt-0.5">🤖 评论 / 评分 / 数据由 AI 生成 · 非真实平台数据</div>
            </div>
          </div>
          <button onClick={() => setExpanded(false)} className="text-stone-400 hover:text-stone-700 text-xs">收起 ✕</button>
        </div>
        ...
        <div className="text-[12px] text-stone-600 mb-4 leading-relaxed">
          假装把这条发到平台 · AI 会模拟<strong className="text-stone-800">真实观众反应</strong>：
          <br/>
          <span className="text-[11px] text-stone-500">· 综合评分（0-100） · 6-10 条真实风格评论 · 点赞 / 评论 / 收藏 / 转发数据 · 24h 热度曲线</span>
        </div>

        {/* 选平台 */}
        <div className="mb-4">
          <div className="text-[10px] tracking-[0.22em] uppercase text-stone-500 font-bold mb-2">选发布平台</div>
          <div className="grid grid-cols-5 gap-1.5">
            <button onClick={() => setChosenPlatform(null)}
              className={`p-2 text-center transition-all border-2 ${chosenPlatform === null ? 'border-purple-500 bg-purple-50' : 'border-stone-200 hover:border-stone-300'}`}
              style={{borderRadius:'3px'}}>
              <div className="text-lg">🤖</div>
              <div className="text-[10px] font-bold mt-0.5">AI 自选</div>
            </button>
            {Object.entries(PLATFORM_THEMES).map(([name, theme]) => (
              <button key={name} onClick={() => setChosenPlatform(name)}
                className={`p-2 text-center transition-all border-2 ${chosenPlatform === name ? 'border-purple-500 bg-purple-50' : 'border-stone-200 hover:border-stone-300'}`}
                style={{borderRadius:'3px'}}>
                <div className="text-lg">{theme.emoji}</div>
                <div className="text-[10px] font-bold mt-0.5">{theme.tag}</div>
              </button>
            ))}
          </div>
        </div>
        ...
        {!initialTranscript && (
          <div className="mt-3 text-[11px] text-amber-700 bg-amber-50 border border-amber-200 px-2.5 py-1.5" style={{borderRadius:'3px'}}>
            ⚠️ 没有转录稿 · AI 只能基于话题做粗略推断。先去 AI 教练复盘里粘贴你的口播稿，效果会更准。
          </div>
        )}
      </Card>
    );
  ...
  const breakdownItems = [
    { k:'hook',       l:'钩子', v: sb.hook ?? 0 },
    { k:'content',    l:'内容', v: sb.content ?? 0 },
    { k:'resonance',  l:'共鸣', v: sb.resonance ?? 0 },
    { k:'virality',   l:'传播', v: sb.virality ?? 0 },
    { k:'completion', l:'完播', v: sb.completion ?? 0 },
  ];
  const totalComments = result.comments?.length || 0;
  const curve = result.trending_curve || [];

  return (
    <Card className="mb-4 border-l-[3px] border-purple-400 overflow-hidden p-0">
      {/* 平台风格头部 */}
      <div className="px-4 py-3 flex items-center justify-between" style={{background: theme.headerBg, color: theme.headerText}}>
        <div className="flex items-center gap-2">
          <span className="text-base">{theme.emoji}</span>
          <span className="font-bold text-sm tracking-wider">{theme.tag} · 模拟反馈</span>
        </div>
        <div className="text-[11px] opacity-80">{result.platform_reason}</div>
      </div>
      ...
      <div className="p-5">
        ...
        <div className="flex items-start gap-3 mb-4">
          <div className="w-12 h-12 flex items-center justify-center text-2xl shrink-0"
            style={{borderRadius:'50%', background: theme.accentBg}}>
            {author.avatar}
          </div>
          <div className="flex-1 min-w-0">
            <div className="flex items-center gap-2 mb-0.5">
              <span className="font-bold text-stone-900 text-sm">{author.name}</span>
              <span className="text-[10px] text-stone-400">· 刚刚 · 发到 {result.platform}</span>
            </div>
            <div className="font-display font-bold text-stone-900 text-base leading-snug">"{result.headline}"</div>
          </div>
        </div>
        ...
        <div className="flex items-stretch gap-3 mb-4">
          ...
          <div className="flex-1 flex flex-col justify-between">
            {result.vibe && VIBE_META[result.vibe] && (
              <div className="self-start px-2.5 py-1 text-[11px] font-bold tracking-wider mb-2"
                style={{
                  borderRadius:'3px',
                  background: VIBE_META[result.vibe].bg,
                  color: VIBE_META[result.vibe].color,
                  border: `1px solid ${VIBE_META[result.vibe].color}`,
                }}>
                {VIBE_META[result.vibe].emoji} {VIBE_META[result.vibe].label}
              </div>
            )}
            {/* 5 维分项 */}
            <div className="space-y-1">
              {breakdownItems.map(it => (
                <div key={it.k} className="flex items-center gap-2">
                  <span className="text-[10px] text-stone-500 w-7 shrink-0">{it.l}</span>
                  <div className="flex-1 h-1.5 bg-stone-100" style={{borderRadius:'1px'}}>
                    <div className="h-full transition-all duration-1000 ease-out"
                      style={{width: `${(it.v / 10) * 100}%`, background: theme.accent, borderRadius:'1px'}} />
                  </div>
                  <span className="text-[10px] tabular-nums font-bold text-stone-700 w-5 text-right">{it.v}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {/* 数据条 */}
        <div className="grid grid-cols-5 gap-1.5 mb-4 py-3 border-y border-stone-200">
          <div className="text-center">
            <div className="text-[9px] text-stone-500 mb-0.5">❤️ 点赞</div>
            <div className="text-sm font-display font-bold tabular-nums" style={{color: theme.accent}}>
              <AnimatedNumber target={result.estimated?.likes || 0} format={(n) => formatCount(Math.round(n))} />
            </div>
          </div>
          <div className="text-center">
            <div className="text-[9px] text-stone-500 mb-0.5">💬 评论</div>
            <div className="text-sm font-display font-bold tabular-nums text-stone-900">
              <AnimatedNumber target={result.estimated?.comments || 0} format={(n) => formatCount(Math.round(n))} />
            </div>
          </div>
          <div className="text-center">
            <div className="text-[9px] text-stone-500 mb-0.5">⭐ 收藏</div>
            <div className="text-sm font-display font-bold tabular-nums text-amber-600">
              <AnimatedNumber target={result.estimated?.saves || 0} format={(n) => formatCount(Math.round(n))} />
            </div>
          </div>
          <div className="text-center">
            <div className="text-[9px] text-stone-500 mb-0.5">📤 转发</div>
            <div className="text-sm font-display font-bold tabular-nums text-stone-900">
              <AnimatedNumber target={result.estimated?.shares || 0} format={(n) => formatCount(Math.round(n))} />
            </div>
          </div>
          <div className="text-center">
            <div className="text-[9px] text-stone-500 mb-0.5">▶️ 完播</div>
            <div className="text-sm font-display font-bold tabular-nums" style={{color: theme.accent}}>
              <AnimatedNumber target={result.estimated?.watch_through || 0} format={(n) => Math.round(n) + '%'} />
            </div>
          </div>
        </div>
        ...
        {curve.length > 0 && (
          <div className="mb-4">
            <div className="flex items-center justify-between mb-1.5">
              <div className="text-[10px] tracking-[0.22em] uppercase text-stone-500 font-bold">📈 发布后 24h 热度</div>
              <div className="text-[10px] text-stone-400">
                {curve.length} 个点 · 峰值 {Math.max(...curve)}%
              </div>
            </div>
            <div className="flex items-end gap-1 h-12 px-1">
              {curve.map((v, i) => (
                <div key={i} className="flex-1 transition-all duration-1000 ease-out"
                  style={{
                    height: `${Math.max(8, v)}%`,
        ...
        {/* 踩雷预警 / 推流爆点 */}
        {(result.potential_pitfall || result.if_pushed_to_top) && (
          <div className="grid sm:grid-cols-2 gap-2 mb-4">
            {result.potential_pitfall && (
              <div className="p-3 bg-red-50 border border-red-200" style={{borderRadius:'3px'}}>
                <div className="text-[10px] tracking-[0.18em] uppercase text-red-700 font-bold mb-1">⚠️ 可能踩雷</div>
                <div className="text-[12px] text-stone-800">{result.potential_pitfall}</div>
              </div>
            )}
            {result.if_pushed_to_top && (
              <div className="p-3 bg-amber-50 border border-amber-200" style={{borderRadius:'3px'}}>
                <div className="text-[10px] tracking-[0.18em] uppercase text-amber-700 font-bold mb-1">🚀 推流爆点</div>
                <div className="text-[12px] text-stone-800">{result.if_pushed_to_top}</div>
              </div>
            )}
          </div>
        )}
        ...
        {totalComments > 0 && (
          <div>
            <div className="flex items-baseline justify-between mb-3 pb-2 border-b border-stone-200">
              <div className="font-bold text-stone-900 text-sm">💬 评论 <span className="text-stone-500 font-normal">({formatCount(result.estimated?.comments || totalComments)})</span></div>
              <div className="text-[10px] text-stone-400">显示 {totalComments} 条</div>
            </div>
            <div className="space-y-3">
              {result.comments.map((c, i) => {
                const meta = STANCE_COLORS[c.stance] || STANCE_COLORS['围观'];
                const avatarBg = ['#FBEFF2', '#F3E8FF', '#FEF3C7', '#D1FAE5', '#EFF6FF', '#F5F0E8'][i % 6];
                return (
                  <div key={i} className="flex items-start gap-2.5">
                    <div className="w-8 h-8 flex items-center justify-center text-base shrink-0"
                      style={{borderRadius:'50%', background: avatarBg}}>
                      {c.avatar || '👤'}
                    </div>
                    <div className="flex-1 min-w-0">
                      <div className="flex items-baseline gap-1.5 flex-wrap mb-0.5">
                        <span className="text-[12px] font-bold text-stone-800">{c.user || '路人'}</span>
                        <span className="text-[9px] tracking-wider font-bold px-1.5 py-0.5"
                          style={{borderRadius:'2px', background: meta.bg, color: meta.color}}>
                          {c.stance}
                        </span>
                        <span className="text-[10px] text-stone-400">· {c.time || '刚刚'}</span>
                      </div>
                      <div className="text-[13px] text-stone-800 leading-relaxed">{c.text}</div>
                      {(c.likes > 0) && (
                        <div className="flex items-center gap-1 mt-1 text-[10px] text-stone-500">
                          <span>❤️</span>
                          <span className="tabular-nums font-medium">{formatCount(c.likes)}</span>
        ...
        <div className="flex gap-2 mt-5 pt-4 border-t border-stone-200 flex-wrap">
          <Btn variant="primary" onClick={makePoster} disabled={posterLoading}>
            {posterLoading ? '生成中...' : '📸 生成晒图'}
          </Btn>
          <Btn variant="secondary" onClick={reset}>重新模拟</Btn>
          <button onClick={() => setExpanded(false)} className="px-3 py-1.5 text-xs text-stone-500 hover:text-stone-800">收起 ✕</button>
        </div>
      </div>
      {posterBlob && <PosterShareModal blob={posterBlob} onClose={() => setPosterBlob(null)}
        fileName={`模拟发布-${result.score || 0}分-${Date.now()}.png`} />}
    </Card>
  );
};
